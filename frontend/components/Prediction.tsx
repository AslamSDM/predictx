...
const Card: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  <div
    className={cn(
      "rounded-lg border text-card-foreground shadow-sm bg-slate-900 border-slate-800",
      className
    )}
    {...props}
  />
...
function Prediction({ data }: Props) {
    ...
    return (
      <div className="bg-slate-950 text-white p-4 md:p-8 font-sans">
        <div className="text-center p-8">
          <p className="text-lg text-slate-400">No prediction data available</p>
        </div>
      </div>
    );
  }
        ...
        try {
          ...
          if (
            ...
            if (wtoken) {
              ...
              console.log("WINNING TOKEN ---------------> ", wtoken);
            }
          }
        } catch (error) {
  ...
  const yesPoolValue = data.yesPool
    ? parseFloat(data.yesPool.toString().replace(/[^0-9.]/g, "")) || 0
    : 0;
  const totalPoolValue = data.totalPool
    ? parseFloat(data.totalPool.toString().replace(/[^0-9.]/g, "")) || 0
    : 0;
  ...
  const handleRedeem = async () => {
    try {
      if (!winningAddress) {
        console.error("No winning token address available");
        return;
      }
        ...
        return (
          <div className="p-6 text-center">
            ...
            {winningAddress ? (
              ...
              >
                Redeem your {outcomeText} tokens
              </button>
            ) : (
              <p className="text-slate-400">
                No winning tokens available for redemption
              </p>
            )}
          </div>
        );
